...
class Boids {
    constructor(num) {
        ...
        for (let i = 0; i < num; i++) {
            const boid = this.generateBoid(
                i,
                this.geometries[i % this.geometries.length],
    ...
    generateBallColors(num_of_materials) {
        return Array.from(
            { length: num_of_materials },
            (_) => Math.random() * 0xffffff
        );
    }
    ...
    generateBoid(idx, geometry, color) {
        ...
        mesh.circlePosition = new THREE.Vector3(
            baseCircleRadius * Math.cos(mesh.circleAngle),
            baseCircleRadius * Math.sin(mesh.circleAngle),
            0
        );

        mesh.color_iter = 0
        mesh.finalColor = new THREE.Color(Math.random() * 0xffffff)
        mesh.otherColor = new THREE.Color(Math.random() * 0xffffff)
        return mesh;
    }
    ...
    tick(args) {
        ...
        const newMode = boids_update_helper(
            this.group,
