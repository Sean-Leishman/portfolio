...
function distance(boid1, boid2) {
    return Math.sqrt(
        (boid1.position.x - boid2.position.x) *
        (boid1.position.x - boid2.position.x) +
        (boid1.position.y - boid2.position.y) *
        (boid1.position.y - boid2.position.y) +
        (boid1.position.z - boid2.position.z) *
        (boid1.position.z - boid2.position.z)
    );
}
...
function flyTowardsCenter(boid, otherBoids) {
    ...
    for (let otherBoid of otherBoids) {
        ...
        if (interprolateColor == null) {
            ...
            //interprolateColor.lerp(otherBoid.material.color, 0.5);
        }
    }
    ...
    //boid.material.color.lerp(interprolateColor, 0.01);
}
...
function convergeBoids(boid, color) {
    ...
    boid.setColor = false;
    if (
        boid.position.distanceTo(boid.circlePosition) > 5 &&
        boid.mode !== "spin"
    ) {
...
function update_boids(scene, boids, boxMesh, mode) {
    if (containment_box == null) {
        containment_box = scene.children.filter((child) => {
            return child.name == "Containment Box";
        })[0];
        avoidance_box = scene.children.filter((child) => {
            return child.name == "Avoidance Box";
        })[0];
    }
    ...
    for (let boid of boids) {
        if (mode === "boids") {
            ...
            boid.iterations = 0;
            if (Math.random() < 0.5) {
                let otherBoids = boids.filter(
                    (otherboid) => distance(otherboid, boid) < visualRange
                );
            ...
            boid.material.color.lerp(boid.finalColor, 0.1)
            if (boid.color_iter == 1 / 0.1) {
                let temp = boid.finalColor
                boid.finalColor = boid.otherColor
                boid.otherColor = temp
                boid.color_iter = 0
            }
            else {
                boid.color_iter++
